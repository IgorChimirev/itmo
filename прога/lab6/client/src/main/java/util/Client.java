...
public class Client {
    ...
    private void processUserPrompt(String command, String arguments) throws IOException, ClassNotFoundException {
        Request request;
        if (command.equalsIgnoreCase("add") || command.equalsIgnoreCase("update") ||  command.equalsIgnoreCase("InsertNull")){
            RouteModel objArgument = VehicleAsker.createElement();
    ...
    private void sendAndReceive(Request request) throws IOException, ClassNotFoundException {
        ...
        ByteBuffer lengthBuffer = ByteBuffer.allocate(4).order(ByteOrder.BIG_ENDIAN); // Указываем порядок байтов
        while (lengthBuffer.hasRemaining()) { // Гарантированно читаем все 4 байта
            int read = channel.read(lengthBuffer);
            if (read == -1) throw new SocketException("Соединение разорвано");
        }
        lengthBuffer.flip();
        int responseLength = lengthBuffer.getInt();

        // Получение основного ответа
        ByteBuffer dataToReceive = ByteBuffer.allocate(responseLength);
        while (dataToReceive.hasRemaining()) { // Читаем до заполнения всего буфера
            int read = channel.read(dataToReceive);
            if (read == -1) throw new SocketException("Соединение разорвано");
        }
        dataToReceive.flip();

        try (ObjectInputStream in = new ObjectInputStream(
                new ByteArrayInputStream(dataToReceive.array(), 0, dataToReceive.limit()))) {
