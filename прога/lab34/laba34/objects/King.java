...
public class King extends Entity implements IHuman {
    ...
    public void performAction(Entity first, Entity second) {
        ...
        switch (currentCondition) {
            ...
            case HUNGRY:
                System.out.println(
                        first.getName() + " голоден, поэтому он едет кушать вместе с " + second.getName() + ".");
                Eat(first, second);
                break;
            case UNHEALTHY:
                System.out.println("У" + first.getName()
                        + " болит голова, поэтому поехал к кататься на лошадях вместе с  " + second.getName() + ".");
    ...
    public void DrivingSomewhere(Entity first, Entity second) {
        ...
        System.out.println(getName() + " поехал вместе с " + second.getName() + " в " + first.getLocation()
                + "чтобы найти что-то выпить");
        System.out.println(getConditionAdjective(second) + " " + second.getName() + " и " + getConditionAdjective(first)
                + first.getName() + " наелись и напились и теперь они весят " + second.getWeight() + " "
                + first.getWeight() + ".");
    ...
    public void Eat(Entity first, Entity second) {
        ...
        System.out.println(this.getName() + " поехал в " + this.getLocation() + " вместе с " + second.getName()
                + " так как " + second.getName() + "позвал его на обед.");
        ...
        System.out.println(this.getName() + "объелся и теперь его вес составляет " + this.getWeight() + ",а "
                + second.getName() + " отправился в " + second.getLocation() + " так как он отравился.");
    }
    ...
    public void HorseWalk(Entity first, Entity second) {
        ...
        System.out.println(first.getName() + " сейчас находится в " + first.getLocation() + " так как "
                + second.getName() + " подвез его.");
    ...
    public void Fight(Entity first, Entity second) {
        ...
        System.out.println(getName() + " поехал в " + this.getLocation() + " вместе с " + second.getName()
                + " так как они посорились в " + first.getLocation());
        System.out.println(
                first.getName() + " сейчас дерутся с " + second.getName() + " на полигоне по правилам рыцарских боев.");
        this.setSize(this.getWeight() - 2);
        this.setHp(2);
        second.setLocation(PlaceConstans.HOSPITAL);
        System.out.println(this.getName() + "похудел и теперь его вес составляет " + this.getWeight() + " а "
                + second.getName() + " отправился в " + second.getLocation() + " так как он проиграл.");
    }
}
