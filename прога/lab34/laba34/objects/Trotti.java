...
public class Trotti extends Entity implements IHuman {
    ...
    public void performAction(Entity first, Entity second) {
        ...
        switch (currentCondition) {
            case HAPPY:
                System.out.println(first.getName() + " хочет прыгать от радости, поэтому пошла гулять в парк вместе с"
                        + second.getName() + "в парк.");
    ...
    public void DrivingSomewhere(Entity first, Entity second) {
        if (Math.random() < 0.5) {
            ...
            System.out.println(this.getName() + "и" + second.getName() + "попали под дождь, поэтому им пришлось поехать"
                    + this.getLocation() + ".");
            ...
            System.out.println(
                    second.getName() + " и " + first.getName() + "радостные поехали в " + first.getLocation() + ".");
        }
    }
    ...
    public void Eat(Entity first, Entity second) {
        ...
        System.out.println(first.getName() + "получил +  " + 10 + " HP из-за того, что  " + second.getName()
                + "накормил его царскими деликатесами.");
